...
compensation = DataFrame(CSV.File("compensation.csv"))
trafos = DataFrame(CSV.File("trafos.csv"))
loads = DataFrame(CSV.File("loads.csv"))
gen = DataFrame(CSV.File("gen.csv"))

# ==============================================================================
# Función optimizada para calcular Ynn y Yns
# ==============================================================================
function calcular_Ynn_Yns(lines, trafos)
    """
    ...
    for k in line_idx
        ...
        yL = 1/(lines.r_pu[k] + lines.x_pu[k]*im)
        Ybus[n1,n1] += yL
        Ybus[n1,n2] -= yL
        Ybus[n2,n1] -= yL
        Ybus[n2,n2] += yL
    ...
    for k in 1:nrow(trafos)
        ...
        yL = 1/(trafos.r_pu[k] + trafos.x_pu[k]*im)
...
function Flujo_punto_fijo(lines, compensation, trafos, loads, gen)
    ...
    Ynn, Yns, _ = calcular_Ynn_Yns(lines, compensation, trafos)
    ...
    for k in 1:nrow(gen)
        n1 = gen.node[k]
        Sn[n1-1] += gen.Pg[k]/sbase+ (gen.Qg[k]/sbase)*1im
    end
    ...
    # Se agrega la compensación reactiva al vector de potencias inyectadas
    for k in 1:nrow(compensation)
        n1 = compensation.node[k]
        Sn[n1-1] += compensation.q_kVAr[k]/sbase*1im
    end
...
# ==============================================================================
# Creando una función para el random walk con números complejos
# ==============================================================================
function random_walk_complex(minutes, nodes, distribution, params)
    ...
    valor_inicial = 0.1 + 0.1*1im
    
    # Inicializar la matriz con el valor inicial en todas las posiciones
    matriz_rw = fill(valor_inicial, minutes, nodes)
    ...
    @views for j in 1:nrow(rw)
        ...
        for k in 1:ncol(rw)
            ...
            if haskey(gen_dict, nodo)
                n = gen_dict[nodo]
                Sn[nodo] = (gen.Pg[n] - real(rw[j,k]))/sbase + (gen.Qg[n] - imag(rw[j,k]))/sbase*im
            else
...
# ==============================================================================
function Ajuste_aut_taps(lines, trafos, rw, gen)
    """
    ...
    """
    Ynn, Yns, _ = calcular_Ynn_Yns(lines,trafos)
        ...
        for j in trafos_c.to
            ...
            while true
                ...
                elseif V < 0.95 && pos < (steps-1)/2
                    ...
                    Ynn, Yns, _ = calcular_Ynn_Yns(lines, trafos_c)
                    Vns_base = flujo_cuasidinamico(Ynn, Yns, rw, gen)
            end
            ind = trafo_dict[j]
            V = abs(Vns_base[j,i])
...
# ==============================================================================
Ynn, Yns, Ybus = calcular_Ynn_Yns(lines, trafos)
minutos, nodes = 60, nrow(loads)
rw = random_walk_complex(minutos, nodes, "normal", Dict("mu" => 0.065, "sigma" => 0.001))
rw_df = DataFrame(rw, ["N$(loads.nodo[i])" for i in 1:nrow(loads)])

# ==============================================================================
# Calculo de pérdidas 
# ==============================================================================
Perd = Vns'*Ybus*Vns*10^5
display(Perd)
